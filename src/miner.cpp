...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("1EurMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("1EurMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("1EurMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("1eur-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running 1EurMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
