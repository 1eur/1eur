...
#if defined(NDEBUG)
#error "1Eur cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
		nSubsidy = 96813500 * COIN;    // %5 premine
	} else if (nHeight > 0 && nHeight < 1000) {
		nSubsidy = 0 * COIN;
	} else if (nHeight >= 1000 && nHeight < 130600) {
		nSubsidy = 60 * COIN;
    } else if (nHeight >= 130600 && nHeight < 259200) {
            nSubsidy = 50 * COIN;
	} else if (nHeight >= 259200 && nHeight < 388800) {
		nSubsidy = 40 * COIN;
	} else if (nHeight >= 388800 && nHeight < 518400) {
		nSubsidy = 30 * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (pindex->nHeight >= 1 && std::time(nullptr) >= 1576731600 && !IsBlockValueValid(block, nExpectedMint, pindex->nMint)) {
