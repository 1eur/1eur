...
*Setup instructions for a gitian build of 1Eur using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the 1Eur
...
Clone the git repositories for 1eur and gitian and then checkout the 1eur version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/1eur-crypto/1eur.git
cd 1eur
...
Follow the instructions in [doc/release-process.md](release-process.md) in the 1eur repository
...
To build 1Eur (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the 1eur repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/1eur/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/1eur-crypto/1eur
...
For example:
```bash
URL=https://github.com/crowning-/1eur.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit 1eur=${COMMIT} --url 1eur=${URL} ../1eur/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit 1eur=${COMMIT} --url 1eur=${URL} ../1eur/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit 1eur=${COMMIT} --url 1eur=${URL} ../1eur/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/1eur-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/1eur-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/1eur-build.assert
```
...
[1eur/gitian.sigs](https://github.com/1eur-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
